...
def plot_neuronPointsCount(pSom, figsize=(15, 15), title="Conteo por nodo"):
    '''
    pSom: trained som object
    This function plots the number of training data points that are associated to each neuron.
    '''

    ## En la variable self.mat_count se encuentra el recuento de puntos
    if not hasattr(pSom,'mat_count'):
        raise AttributeError('pSom is not trained yet.')
    ...
    # Remove the axes
    ax.axis('off')
    vicinity = pSom.vicinity

    if vicinity == 'rectangular':
        ...
        for row in range(pSom.nrows):
            for col in range(pSom.ncols):
                xH,yH = neuronConvert_Grid_to_R2(pSom.vicinity,row, col)
                countValueNeuron = pSom.mat_count[row, col]
                hexa = graphHexagon(xH,yH,color=cmap(norm(countValueNeuron)))
                ax.add_patch(hexa)
                plt.text(xH, yH, str(countValueNeuron), color='k', va="center", ha="center")
...
def graphHexagon(xH,yH,
                 radius = 1,
                 color=None,
                 text=None):
    '''
    return a Hexagon patch
    '''
    hexa = patches.RegularPolygon((xH, yH), 6,
                                  radius=radius,
                                  orientation=0,
                                  facecolor=color,
                                  # facecolor='red',
...
def plot_neurPointsCount(pSom, figsize=(15, 15), title="Conteo por nodo"):
    '''
    plots the number of data points that are associated to each neuron.
    '''
